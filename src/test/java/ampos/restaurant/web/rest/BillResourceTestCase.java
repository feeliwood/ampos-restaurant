...
public class BillResourceTestCase extends BaseTestCase {
    ...
    public void createBillTestCase() throws IOException, Exception {
        ...
        MvcResult result = mockMvc.perform( post( "/bills" ).contentType( MimeTypeUtils.APPLICATION_JSON_VALUE ) ).andExpect( status().is( 201 ) ).andReturn();
        // check database
    ...
    public void createBillItemTestCase() throws IOException, Exception {
        ...
        // compare
        MvcResult result = mockMvc.perform( post( "/bills/1/bill-items" ).contentType( MimeTypeUtils.APPLICATION_JSON_VALUE ).content( asJsonString( billItems ) ) ).andExpect( status().is( 201 ) ).andReturn();
    ...
    public void updateBillItemTestCase() throws IOException, Exception {
        ...
        MvcResult result = mockMvc.perform( put( "/bills/1/bill-items/1" ).contentType( MimeTypeUtils.APPLICATION_JSON_VALUE ).content( "2" ) ).andExpect( status().is( 200 ) ).andReturn();
        BillItemsResponse actualData = jsonToObject( result.getResponse().getContentAsString(), BillItemsResponse.class );
